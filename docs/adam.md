...
square gt (cid:12) gt. Good default settings for the tested machine learning problems are α = 0.001,
β1 = 0.9, β2 = 0.999 and (cid:15) = 10−8. All operations on vectors are element-wise. With βt
1 and βt
2
...
(cid:98)vt + (cid:15)) (Update parameters)
...
terested in minimizing the expected value of this function, E[f (θ)] w.r.t.
its parameters θ. With
...
estimates (cid:98)mt and (cid:98)vt. See section 3 for more details.
...
An important property of Adam’s update rule is its careful choice of stepsizes. Assuming (cid:15) = 0, the
(cid:98)vt. The effective stepsize has
effective step taken in parameter space at timestep t is ∆t = α · (cid:98)mt/
1 − β2, and |∆t| ≤ α
...
(cid:98)vt| < 1 therefore |∆t| < α. In
(cid:98)vt ≈ ±1 since |E[g]/(cid:112)E[g2]| ≤ 1. The effective
more common scenarios, we will have that (cid:98)mt/
magnitude of the steps taken in parameter space at each timestep are approximately bounded by
the stepsize setting α, i.e., |∆t| (cid:47) α. This can be understood as establishing a trust region around
...
(cid:98)vt the signal-to-noise ratio (SN R). With a smaller SNR the effective
we will call the ratio (cid:98)mt/
stepsize ∆t will be closer to zero. This is a desirable property, since a smaller SNR means that
there is greater uncertainty about whether the direction of (cid:98)mt corresponds to the direction of the true
...
∆t is also invariant to the scale of the gradients; rescaling the gradients g with factor c will scale (cid:98)mt
with a factor c and (cid:98)vt with a factor c2, which cancel out: (c · (cid:98)mt)/(

c2 · (cid:98)vt) = (cid:98)mt/
...
vt = β2 · vt−1 + (1 − β2) · g2
t indicates the elementwise square gt (cid:12) gt) can be written as
...
learning problem. We also use some deﬁnitions simplify our notation, where gt (cid:44) ∇ft(θt) and gt,i
as the ith element. We deﬁne g1:t,i ∈ Rt as a vector that contains the ith dimension of the gradients
over all iterations till t, g1:t,i = [g1,i, g2,i, · · · , gt,i]. Also, we deﬁne γ (cid:44) β2
. Our following
1√
β2
...
Theorem 4.1. Assume that the function ft has bounded gradients, (cid:107)∇ft(θ)(cid:107)2 ≤ G, (cid:107)∇ft(θ)(cid:107)∞ ≤
G∞ for all θ ∈ Rd and distance between any θt generated by Adam is bounded, (cid:107)θn − θm(cid:107)2 ≤ D,
(cid:107)θm − θn(cid:107)∞ ≤ D∞ for any m, n ∈ {1, ..., T }, and β1, β2 ∈ [0, 1) satisfy β2
< 1. Let αt = α√
1√
β2
t
...
(cid:112)T (cid:98)vT,i << dG∞

Our Theorem 4.1 implies when the data features are sparse and bounded gradients, the sum-
mation term can be much smaller than its upper bound (cid:80)d
T and
(cid:80)d
T , in particular if the class of function and data features are in the form of
section 1.2 in (Duchi et al., 2011). Their results for the expected value E[(cid:80)d
i=1 (cid:107)g1:T,i(cid:107)2] also apply
T ),
...
i=1 (cid:107)g1:T,i(cid:107)2 << dG∞
...
Corollary 4.2. Assume that the function ft has bounded gradients, (cid:107)∇ft(θ)(cid:107)2 ≤ G, (cid:107)∇ft(θ)(cid:107)∞ ≤
G∞ for all θ ∈ Rd and distance between any θt generated by Adam is bounded, (cid:107)θn − θm(cid:107)2 ≤ D,
(cid:107)θm − θn(cid:107)∞ ≤ D∞ for any m, n ∈ {1, ..., T }. Adam achieves the following guarantee, for all
T ≥ 1.
...
This result can be obtained by using Theorem 4.1 and (cid:80)d
limT →∞
...
preconditioner that adapts to the geometry of the data, since (cid:98)vt is an approximation to the diagonal
...
inﬁnitesimally close to 1 from below, then limβ2→1 (cid:98)vt = t−1 · (cid:80)t
i=1 g2
t . AdaGrad corresponds to a
version of Adam with β1 = 0, inﬁnitesimal (1 − β2) and a replacement of α by an annealed version
αt = α · t−1/2, namely θt − α · t−1/2 · (cid:98)mt/(cid:112)limβ2→1 (cid:98)vt = θt − α · t−1/2 · gt/
t =
θt − α · gt/
...
051015202530354045iterations over entire dataset0.20.30.40.50.60.7training costMNIST Logistic RegressionAdaGradSGDNesterovAdam020406080100120140160iterations over entire dataset0.200.250.300.350.400.450.50training costIMDB BoW feature Logistic RegressionAdagrad+dropoutRMSProp+dropoutSGDNesterov+dropoutAdam+dropoutPublished as a conference paper at ICLR 2015
...
faster than Adagrad for CNNs shown in Figure 3 (right). We notice the second moment estimate (cid:98)vt
vanishes to zeros after a few epochs and is dominated by the (cid:15) in algorithm 1. The second moment
...
050100150200iterations over entire dataset10-210-1training costMNIST Multilayer Neural Network + dropoutAdaGradRMSPropSGDNesterovAdaDeltaAdam0.00.51.01.52.02.53.0iterations over entire dataset0.51.01.52.02.53.0training costCIFAR10 ConvNet First 3 EpochesAdaGradAdaGrad+dropoutSGDNesterovSGDNesterov+dropoutAdamAdam+dropout051015202530354045iterations over entire dataset10-410-310-210-1100101102training costCIFAR10 ConvNetAdaGradAdaGrad+dropoutSGDNesterovSGDNesterov+dropoutAdamAdam+dropoutPublished as a conference paper at ICLR 2015
...
β1=0β1=0.9β2=0.99β2=0.999β2=0.9999β2=0.99β2=0.999β2=0.9999(a) after 10 epochs(b) after 100 epochslog10(α)LossPublished as a conference paper at ICLR 2015
...
Initalization bias can again be corrected by the estimator (cid:98)θt = ¯θt/(1 − βt
...
Roux, Nicolas L and Fitzgibbon, Andrew W. A fast natural newton method.
...
plify our notation, where gt (cid:44) ∇ft(θt) and gt,i as the ith element. We deﬁne g1:t,i ∈ Rt as a vector
that contains the ith dimension of the gradients over all iterations till t, g1:t,i = [g1,i, g2,i, · · · , gt,i]
Lemma 10.3. Let gt = ∇ft(θt) and g1:t be deﬁned as above and bounded, (cid:107)gt(cid:107)2 ≤ G, (cid:107)gt(cid:107)∞ ≤
G∞. Then,
...
T,i ≤ (cid:107)g1:T,i(cid:107)2 −

≤ (cid:107)g1:T,i(cid:107)2 −

g2
T,i
2(cid:107)g1:T,i(cid:107)2
g2
T,i
2(cid:112)T G2
...
Lemma 10.4. Let γ (cid:44) β2
1√
β2
(cid:107)gt(cid:107)∞ ≤ G∞, the following inequality holds

. For β1, β2 ∈ [0, 1) that satisfy β2
1√
β2

< 1 and bounded gt, (cid:107)gt(cid:107)2 ≤ G,
...
For γ < 1, using the upper bound on the arithmetic-geometric series, (cid:80)
...
To simplify the notation, we deﬁne γ (cid:44) β2
1√
β2
learning rate αt is decaying at a rate of t− 1
exponentially with λ, that is typically close to 1, e.g. 1 − 10−8.
Theorem 10.5. Assume that the function ft has bounded gradients, (cid:107)∇ft(θ)(cid:107)2 ≤ G, (cid:107)∇ft(θ)(cid:107)∞ ≤
G∞ for all θ ∈ Rd and distance between any θt generated by Adam is bounded, (cid:107)θn − θm(cid:107)2 ≤ D,
...
2 ≤ (cid:107)g1:t,i(cid:107)2 and β1,t ≤ β1. Then
...
From the assumption, (cid:107)θt − θ∗(cid:107)2 ≤ D, (cid:107)θm − θn(cid:107)∞ ≤ D∞, we have:
